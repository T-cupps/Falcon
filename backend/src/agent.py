# ======================================================
# ✨ COZY COFFEE SHOP VOICE AGENT 
# 🌸 Friendly example agent for a coffee shop voice assistant
# 🧸 Clean, comfy, and ready to use
# ======================================================
...
print("\n" + "🧸" * 50)
print("☁️ COZY COFFEE SHOP AGENT")
...
@dataclass
class OrderState:
    """☕ Cozy coffee order state with validation"""
...
async def set_drink_type(
    ...
    return f"☕ Lovely choice! One {drink} coming right up!"
...
async def set_size(
    ...
    print(f"📝 Order Progress: {ctx.userdata.order.get_summary()}")
    return f"📏 {size.title()} — perfect for your {ctx.userdata.order.drinkType}!"
...
async def set_milk(
    ...
    if milk == "none":
        return "🥛 Got it — black coffee, bold and simple!"
    return f"🥛 {milk.title()} milk — lovely!"
...
async def set_extras(
    ...
    return "✨ No extras — keeping it cozy and classic!"
...
async def set_name(
    ...
    print(f"📝 Order Progress: {ctx.userdata.order.get_summary()}")
    return f"👤 Wonderful, {ctx.userdata.order.name}! Almost ready to finish your order!"
...
⏰ We'll prepare it now — should be ready in a few minutes!

💖 Thanks for visiting our cozy cafe — enjoy your drink!"""
        
    except Exception as e:
        print(f"❌ ORDER SAVE FAILED: {e}")
        return "⚠️ Order recorded but there was a small issue saving it. We'll still make your drink!"
...
class BaristaAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            🏪 You are a FRIENDLY and PROFESSIONAL barista at "Cozy Corner Cafe".
            ...
            - Use cute emojis to make it friendly
...
async def entrypoint(ctx: JobContext):
    ...
    print("\n" + "🏪" * 25)
    print("☁️ BREW & BEAN CAFE - AI BARISTA")
    print("🧸 Welcome! Ready to take cozy orders.")
...
if __name__ == "__main__":
    print("\n" + "⚡" * 25)
    print("🎬 STARTING COZY COFFEE SHOP AGENT...")
    print("💖 Enjoy a warm and friendly voice experience.")
    print("⚡" * 25 + "\n")
