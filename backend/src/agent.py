    ...
    try:
        ...
        if cur.fetchone()[0] == 0:
            catalog = [
                ...
                ("chips-lays", "Lays", "Snacks", 20.00, "Lays", "50g", "pack", json.dumps(["snack", "spicy"])),
                ("chips-kurkure", "Kurkure", "Snacks", 20.00, "Kurkure", "50g", "pack", json.dumps(["snack", "spicy"])),
                ("chips-bingo", "Bingo Mad Angles Strong Masala", "Snacks", 25.00, "Bingo", "55g", "pack", json.dumps(["snack", "crispy"])),
                ("chips-uncle", "Uncle Chipps Classic Salted", "Snacks", 18.00, "Uncle Chipps", "50g", "pack", json.dumps(["snack"])),
                
                # Chocolates & Sweets
                ("choc-cadbury-65g", "Cadbury Dairy Milk 65g", "Confectionery", 45.00, "Cadbury", "65g", "bar", json.dumps(["chocolate", "sweet"])),
                ("choc-kitkat-4f", "KitKat", "Confectionery", 30.00, "Nestle", "41.5g", "pack", json.dumps(["chocolate", "crispy"])),
                ("choc-mars", "Mars Chocolate Bar", "Confectionery", 40.00, "Mars", "51g", "bar", json.dumps(["chocolate"])),
                ("choc-bournville", "Cadbury Bournville 80g", "Confectionery", 80.00, "Cadbury", "80g", "bar", json.dumps(["dark chocolate"])),
                
                # Energy / Energy-style Drinks
                ("drink-redbull-250ml", "Red Bull", "Beverages", 120.00, "Red Bull", "250ml", "can", json.dumps(["energy", "drink"])),
                ("drink-monster-500ml", "Monster Energy 500ml", "Beverages", 220.00, "Monster", "500ml", "can", json.dumps(["energy", "drink"])),
                ("drink-sting-500ml", "Sting Power + 500ml", "Beverages", 60.00, "Sting", "500ml", "bottle", json.dumps(["energy", "drink"])),
                ("drink-coke-zero-500ml", "Coke Zero Sugar 500ml", "Beverages", 50.00, "Coca-Cola", "500ml", "bottle", json.dumps(["softdrink"])),
            ...
            """, catalog)
            conn.commit()
            logger.info(f"Seeded Indian catalog into {get_db_path()} ☕️")
...
async def simulate_delivery_flow(order_id: str):
    ...
    """
    logger.info(f"🧺 [Simulation] Started tracking for order {order_id}")
...
async def place_order(
    ...
    return f"Order placed successfully! Order ID: {order_id}. Total: \u20B9{total:.2f}. I'll keep an eye on it and update the status shortly. 🫶"
...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Daisy', a gentle and helpful assistant for 'Cozy Corner Grocery', a friendly neighbourhood Indian grocery shop.
            ...
            4. Orders: Place orders and check status.
            5. Cancellation: You can CANCEL an order if the user asks, provided it's not delivered yet.

            When placing an order, be warm and encouraging — let the user know you'll keep an eye on the order and update it automatically.
            If user asks "Where is my order?", check status and respond kindly.
            """,
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🌿 " * 12)
    logger.info("✨ STARTING COZY CORNER GROCERY (Indian Context + Auto-Tracking)")
